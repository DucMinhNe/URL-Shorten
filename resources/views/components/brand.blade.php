...
@php
    $textColor = $variant === 'light' ? 'text-canvas' : 'text-ink-deep';
    $dot = $variant === 'light' ? '#FFD60A' : '#0064E0';
    $h = $size === 'lg' ? 28 : 22;
@endphp
...
<a href="{{ route('home') }}" {{ $attributes->merge(['class' => "inline-flex items-center gap-2 {$textColor}"]) }}>
    <svg width="{{ $h }}" height="{{ $h }}" viewBox="0 0 32 32" fill="none">
        <rect x="2" y="2" width="28" height="28" rx="8" fill="{{ $variant === 'light' ? '#FFFFFF' : '#0A1317' }}"/>
    ...
    <span class="type-link-md tracking-tight">LinkPay</span>
</a>
